...
module.exports = function(appUrl) {
  ...
  var slugAndMinimize = function(conf, next) {
    ...
    var confUrl = appUrl + 'conference/view/' + conf.slug;
    var mnzdTotal = 0;
  ...
  var findAllConferences = function(callback) {
    ConfModel.find({}, ['slug', 'title', 'start'], callback).sort({start:-1});
  };

  var findConference = function(confSlug, callback) {
   ConfModel.findOne({'slug':confSlug}, function(err, conf) {
     if (conf) {
       storeDateRange(conf, function() {});
     }
     callback(err, conf);
   });
  };
  ...
  var findTalk = function(confSlug, talkSlug, callback) {
    findConference(confSlug, function(err, conf) {
      if (err) {
        callback(err);
        return;
      }
      ...
      var talks = conf.talks;
      for (var i = 0; i < talks.length; i++) {
        if (talks[i].slug === talkSlug) {
          callback(null, talks[i]);
          return;
        }
      }
      callback(null, null);
