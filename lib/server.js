module.exports = function(express, everyauth, nowjs, model, controllers) {
  ...
  everyauth.twitter
    ...
    .findOrCreateUser( function (session, accessToken, accessSecret, twitUser) {
      return findOrCreateUser(twitUser);
    })
    .redirectPath('/');
  ...
  var adr = app.address();
  var appUrl = 'http://latest-build.nko2.nodeknockout.com/'; //'http://' + adr.address + ((adr.port === 80) ? '' : ':' + adr.port) + '/';
...
function findOrCreateUser(twitUser) {
